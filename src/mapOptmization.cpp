...
public:
    ...
    pcl::PointCloud<PointType>::Ptr cloudKeyPoses3D; // 储存所有关键帧的位姿信息，3D位姿形式  用于快速查找空间上邻近的关键帧（例如在回环检测、局部地图构建时）
    pcl::PointCloud<PointTypePose>::Ptr cloudKeyPoses6D; // 存储所有关键帧的 6D 位姿（x, y, z, roll, pitch, yaw） + 时间戳
    ...
    // 用于点到线（corner）或点到面（surf）优化（scan-to-map registration）过程中的中间数据容器
